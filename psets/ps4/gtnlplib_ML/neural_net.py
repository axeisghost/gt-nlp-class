...
class VanillaWordEmbeddingLookup(nn.Module):
    ...
    def forward(self, sentence):
        ...
        embeds = [self.word_embeddings(i) for i in inp]  # store each Variable in here
        # STUDENT
        # END STUDENT
        return embeds
...
class BiLSTMWordEmbeddingLookup(nn.Module):
    ...
    def __init__(self, word_to_ix, word_embedding_dim, hidden_dim, num_layers, dropout):
        ...
        # 2. The LSTM
        self.lstm = nn.LSTM(word_embedding_dim, hidden_dim/2, num_layers = num_layers, \
    ...
    def forward(self, sentence):
        ...
        # STUDENT
        embeds = [self.word_embeddings(i) for i in inp]
        embeds = torch.cat(embeds)
        embeds = embeds.view(len(sentence), 1, self.word_embedding_dim)
        out, self.hidden = self.lstm(embeds)
        #out = [x.view(1, self.word_embedding_dim) for x in list(out)]

        out = list(out)
        for i in range(len(out)):
            out[i] = out[i].view(1, self.hidden_dim)
            # print out[i].size()
        return out
        # END STUDENT
...
class MLPCombinerNetwork(nn.Module):
    ...
    def forward(self, head_embed, modifier_embed):
        ...
        """
        # STUDENT
        data = utils.concat_and_flatten([head_embed, modifier_embed])
        out = F.tanh(self.linear1(data))
...
class LSTMCombinerNetwork(nn.Module):
    ...
    def __init__(self, embedding_dim, num_layers, dropout):
        ...
        # STUDENT
        self.lstm = nn.LSTM(embedding_dim * 2, embedding_dim, num_layers = num_layers, \
    ...
    def forward(self, head_embed, modifier_embed):
        ...
        # print data.size()
        data = data.view(1, 1, self.embedding_dim * 2)
...
class ActionChooserNetwork(nn.Module):
    ...
    def forward(self, inputs):
        ...
        data = utils.concat_and_flatten(inputs)
