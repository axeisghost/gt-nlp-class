...
class ParserState:
    ...
    def done_parsing(self):
        ...
        """
        # STUDENT
        return len(self.stack) == 1 and END_OF_INPUT_TOK == self.input_buffer[self.curr_input_buff_idx].headword
        # END STUDENT
    ...
    def _reduce(self, action):
        ...
        if Actions.REDUCE_R == action:
            modword, modPos, modEmbed = self.stack.pop()
            headword, headPos, headEmbed = self.stack.pop()
        else:
            headword, headPos, headEmbed = self.stack.pop()
            modword, modPos, modEmbed = self.stack.pop()
        combinedEmbed = self.combiner(headEmbed, modEmbed)
        self.stack.append(StackEntry(headword, headPos, combinedEmbed))
        return DepGraphEdge((headword, headPos), (modword, modPos))
...
class TransitionParser(nn.Module):
    ...
    def forward(self, sentence, actions=None):
        ...
        while parser_state.done_parsing() is False:
            features = self.feature_extractor.get_features(parser_state)
            logProbs = self.action_chooser(features)
            outputs.append(logProbs)
            if len(action_queue) > 0:
                action = action_queue.popleft()
                if 0 == action:
                    actions_done.append(Actions.SHIFT)
                    dep_graph.add(parser_state.shift())
                elif 1 == action:
                ...
                action = utils.argmax(logProbs)
                if 0 == action:
                    token = parser_state.input_buffer_peek_n(1)[0].headword
                    if END_OF_INPUT_TOK == token:
                        actions_done.append(Actions.REDUCE_R)
                        dep_graph.add(parser_state.reduce_right())
                    else:
                        actions_done.append(Actions.SHIFT)
