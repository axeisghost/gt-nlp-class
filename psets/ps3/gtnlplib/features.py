...
def word_feats_competitive_en(words,y,y_prev,m):
    ...
    if m < len(words) - 1:
        ...
        fv[(y, constants.NEXT_WORD_FEAT, constants.POST_END_TOKEN)] = 1.0
    fv[(y, constants.SUFFIX_FEAT, words[m][-2:])] = 1.0
    fv[(y, "--LENGTH--", str(len(words[m])))] = 0.2
    return fv
...
def word_feats_competitive_ja(words,y,y_prev,m):
    ...
    if m < len(words) - 1:
        ...
        fv[(y, constants.NEXT_WORD_FEAT, constants.POST_END_TOKEN)] = 1.0
    fv[(y, "--LENGTH--", str(len(words[m])))] = 0.5
    fv[(y, "--PREFIX-FEAT--", words[m][:2])] = 1.0
    fv[(y, constants.SUFFIX_FEAT, words[m][-2:])] = 1.0
    return fv
...
def hmm_feats_competitive_en(words,y,y_prev,m):
    ...
    fv.update(word_feats_competitive_en(words,y,y_prev,m))
    return fv

def hmm_feats_competitive_ja(words,y,y_prev,m):
    fv = hmm_feats(words,y,y_prev,m)
    fv.update(word_feats_competitive_ja(words,y,y_prev,m))
    return fv
